...
local function setupPlayer(Player)
    -- On a cold server start the inventory can initialise before qb-core; wait
    -- for the core object so AddPlayerMethod calls below never run against nil.
...
CreateThread(function()
    -- Wait for qb-core to be started before grabbing the core object. Using a
    -- fixed timeout here is the classic cause of the inventory failing to load
    -- on a fresh server start (it only worked after restarting the resource).
    while GetResourceState('qb-core') ~= 'started' do Wait(50) end
