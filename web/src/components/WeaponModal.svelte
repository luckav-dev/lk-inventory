<script lang="ts">
  ...
  const socketRows = [
    { key: 'optic', label: 'MIRA (OPTIC)', column: 'left' },
    { key: 'muzzle', label: 'SILENCIADOR (MUZZLE)', column: 'left' },
    { key: 'flashlight', label: 'LINTERNA (FLASHLIGHT)', column: 'right' },
    { key: 'magazine', label: 'CARGADOR (CLIP)', column: 'right' },
  ];
    ...
    <div class="weapon-mod-container">
      ...
      <div class="weapon-mod-content-tactical">
        <div class="weapon-mod-column sockets-column">
          <span class="column-title">RANURAS SUPERIORES</span>
          <div class="socket-stack">
            {#each socketRows.filter((row) => row.column === 'left') as row}
              ...
              <div class="socket-slot" data-socket={row.key}>
                ...
                {:else}
                  <div class="socket-item empty"><span class="empty-socket-placeholder">RANURA VACIA</span></div>
                {/if}
              </div>
            {/each}
          </div>
        ...
        <div class="weapon-mod-column stats-column">
          <span class="column-title">ACCESORIOS ADICIONALES</span>
          <div class="socket-stack">
          ...
          {#if availableComponents.length}
            <div class="inventory-components-panel">
              <span class="column-title inventory-components-title">EN INVENTARIO</span>
      ...
      <div class="weapon-mod-footer">
        <p>USA UN ACCESORIO COMPATIBLE PARA INSTALARLO. CLICK EN RANURA PARA DESEQUIPAR.</p>
      </div>
    </div>
  </div>
{/if}
