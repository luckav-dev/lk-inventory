...
local function syncCurrentWeaponSlot()
	SendNUIMessage({
		action = 'setCurrentWeapon',
		data = currentWeapon and currentWeapon.slot or nil
	})
end
...
RegisterNetEvent('lk_inventory:OpenStashWithPin', function(stashName)
	return client.openInventory('stash', stashName)
end)
...
exports('OpenStashWithPin', function(stashName)
	return client.openInventory('stash', stashName)
end)
...
local function registerCommands()
	...
	RegisterCommand('inventoryconfig', function()
		SendNUIMessage({ action = 'openConfig' })
		SetNuiFocus(true, true)
	end, false)
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	SendNUIMessage({
		...
		data = {
			...
			defaultlayout = client.defaultlayout
		}
	})
...
RegisterNUICallback('lk_inventory:checkStashPin', function(data, cb)
	local response = lib.callback.await('lk_inventory:checkStashPin', false, data and data.stash)
	cb(response or { required = false, unlocked = true })
end)

RegisterNUICallback('lk_inventory:unlockStashPin', function(data, cb)
	local response = lib.callback.await('lk_inventory:unlockStashPin', false, data)
...
RegisterNUICallback('lootAllComplete', function(data, cb)
	TriggerEvent('lk_inventory:lootAllComplete', data and data.count or 0)
	cb(1)
end)
